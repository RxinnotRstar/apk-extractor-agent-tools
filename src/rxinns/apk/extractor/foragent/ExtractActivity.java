package rxinns.apk.extractor.foragent;
 ...
 *
 * 传入方式（任一）：
 *   1. am start -n rxinns.apk.extractor.foragent/.ExtractActivity --es pkg com.example.app
 *   2. am start -n rxinns.apk.extractor.foragent/.ExtractActivity -d "pkg://com.example.app"
 *   3. am start -a rxinns.apk.extractor.foragent.EXTRACT --es pkg com.example.app
...
public class ExtractActivity extends Activity {
    ...
    private void doExtract(String pkg) {
        ...
        // 收集所有 apk：base + split
        //
        // 注意：ApplicationInfo.splitSourceDirs 是 API 21(Android 5.0) 才加进去的字段。
        // 直接写 ai.splitSourceDirs 在更老的系统上会抛 NoSuchFieldError
        // （Error 不是 Exception，不接就是进程崩），所以只能用反射取。
        // 老系统本来也没有 split apk 这回事，取不到就返回 null，走纯 base.apk 逻辑。
        List<String> paths = new ArrayList<String>();
        try {
            java.lang.reflect.Field sf =
                    ApplicationInfo.class.getField("splitSourceDirs");
            String[] splits = (String[]) sf.get(ai);
            if (splits != null) {
                for (int i = 0; i < splits.length; i++) {
                    if (splits[i] != null) paths.add(splits[i]);
                }
            }
        } catch (Throwable t) {
            Log.i(TAG, "取不到 splitSourceDirs（老系统），当没有 split 处理: " + t);
        }
